...
public class TestCase10 extends AFMainTest implements AFTestCase {

	public String TestName() {
		return "Too much Resources";
	}

	public String getDescription() {
		StringBuffer sbf = new StringBuffer();
		sbf.append("AF send AAR (pre) with offer and answer (several codecs) for Auido\n");
		sbf.append("AF send AAR (final) with offer and answer (only 1 codec) with more Bandwidth requirements for Auido\n");
	...
	public boolean startTest() throws Exception {
		...
		sd.getAnswer().audio.bandwidth=10000000;
		sd.getOffer().audio.status=Media.ACTIVE;
		sd.getOffer().audio.codec=singleCodec;
		sd.getOffer().audio.bandwidth=10000000;
		aar = getRxAAR();
		this.addMedia(aar, sd);
		LOGGER.info(TestName()+" send AAR!");
