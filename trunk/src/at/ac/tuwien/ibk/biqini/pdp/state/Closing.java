...
public class Closing extends State {
	...
	public Closing(StateMachine machine) {
		super(machine);
	...
	protected State incomingAnswer(String _fqdn, DiameterAnswer _ans, DiameterRequest _req) {
		if (_ans instanceof GxRAA){
			return new Terminated(stateMachine);
		}
		return this;
	}
	...
	protected State incomingRequest(String _fqdn, DiameterRequest _req) {
		if (_req instanceof RxSTR)
			gx.removeSession(stateMachine.getPdpSession());
			systemStructure.killSession(stateMachine.getPdpSession());
		return this;
	}
