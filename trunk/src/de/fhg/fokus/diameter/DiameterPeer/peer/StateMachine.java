/*
 * $Id: StateMachine.java 27 2006-11-27 09:43:28Z shenny $
 *
...
//				break;
	...
	private static int Process_CER(Peer p,DiameterMessage cer)
	{
		...
		while(i.hasNext()&& common_app==0){
			avp = i.next();
			switch (avp.getAVPCode()){
				case AVP.Auth_Application_Id:
					p.AuthApp.add(new Application(avp.getIntData(),0,Application.Auth));
					i2 = p.diameterPeer.AuthApp.iterator();
					while(i2.hasNext()){
						app = i2.next();
						if (avp.getIntData()==Application.Relay ||
							(app.id==avp.getIntData() && app.vendor==0)){
							common_app++;
							break;
						}
					}
				...
				case AVP.Acct_Application_Id:
					p.AcctApp.add(new Application(avp.getIntData(),0,Application.Acct));
				...
				case AVP.Vendor_Specific_Application_Id:
					...
					if (avp2!=null) {
						p.AuthApp.add(new Application(avp2.getIntData(),avp_vendor.getIntData(),Application.Auth));
						i2 = p.diameterPeer.AuthApp.iterator();
						while(i2.hasNext()){
							app = i2.next();
							if (avp2.getIntData()==Application.Relay ||
								(app.id==avp2.getIntData() && app.vendor==avp_vendor.getIntData())){
								common_app++;
								break;
							}
						}
					...
					if (avp2!=null) {
						p.AcctApp.add(new Application(avp2.getIntData(),avp_vendor.getIntData(),Application.Acct));
						i2 = p.diameterPeer.AcctApp.iterator();
						while(i2.hasNext()){
							app = i2.next();
							if (avp2.getIntData()==Application.Relay ||
