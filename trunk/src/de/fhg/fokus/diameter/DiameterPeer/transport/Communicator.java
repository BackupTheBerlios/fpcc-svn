/*
 * $Id: Communicator.java 2 2006-11-14 22:37:20Z vingarzan $
 *
		...
		switch (msg.commandCode){
			case DiameterMessage.Code_CE:
				if (msg.flagRequest) {
					...
					p = diameterPeer.peerManager.getPeerByFQDN(new String(fqdn.getData()));
					if (p==null) {
						p = diameterPeer.peerManager.addDynamicPeer(new String(fqdn.getData()),
									new String(realm.getData()));
					}
					if (p==null){
						//Give up
