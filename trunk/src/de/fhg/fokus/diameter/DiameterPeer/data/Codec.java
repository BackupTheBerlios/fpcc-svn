/*
 * $Id: Codec.java 2 2006-11-14 22:37:20Z vingarzan $
 *
...
public class Codec {
	...
	public static AVP decodeAVP(byte[] from,int start) throws AVPDecodeException
	{
		...
		/*Changed by Christoph Egger
		 * call Factory to create object of desired type*/
		/*Changed by Marco Happenhofer
		 * we need the vendorID*/
		
		
		boolean flag_vendor_specific = 	(from[start+4] & 0x80)!=0;
		boolean flag_mandatory = 		(from[start+4] & 0x40)!=0;
		boolean flag_protected = 		(from[start+4] & 0x20)!=0;
		...
		to = AVPFactory.parse(code,vendorID);
		if(to==null)
		{
			to = new AVP();
			to.code = code;
			to.flag_mandatory=flag_mandatory;
			
			to.flag_vendor_specific=flag_vendor_specific;
			to.vendor_id=vendorID;
		}
		to.flag_protected=flag_protected;
	...
	public static DiameterMessage decodeDiameterMessage(byte[] from,int start) throws DiameterMessageDecodeException
	{
		...
		DiameterMessage ret = MessageFactory.parse(to.applicationID, to.flagRequest, to.commandCode);
		if(ret == null)
		{
			ret=to;
		}
		...
		{
			ret.version= to.version;
			ret.flagProxiable = to.flagProxiable;
			ret.flagRetransmission = to.flagRetransmission;
			ret.flagError = to.flagError;
			ret.hopByHopID = to.hopByHopID;
			ret.endToEndID = to.endToEndID;
		}
		i = start+20;
		while (i<start+len){
			try {
	...
	public static byte[] encodeDiameterMessage(DiameterMessage from)
	{
		...
		for(i=0;i<from.avps.size();i++){
			t = Codec.encodeAVP(from.avps.get(i));
			temp.add(t);
			len += t.length;
		}
		total_len=20+len;
